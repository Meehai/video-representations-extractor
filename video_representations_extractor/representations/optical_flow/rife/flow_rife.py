...
class FlowRife(Representation):
	def __init__(self, name, dependencies, saveResults:str, dependencyAliases, computeBackwardFlow:bool):
		...
		self.weightsDir = Path(f"{os.environ['VRE_WEIGHTS_DIR']}/rife").absolute()
	...
	def get(self, t_source, t_target) -> np.ndarray:
		...
		I0 = tr.from_numpy(np.transpose(frame1, (2,0,1))).to(device, non_blocking=True).unsqueeze(0).float() / 255.
		I1 = tr.from_numpy(np.transpose(frame2, (2,0,1))).to(device, non_blocking=True).unsqueeze(0).float() / 255.
		n, c, h, w = I0.shape
		ph = ((h - 1) // 32 + 1) * 32
		pw = ((w - 1) // 32 + 1) * 32
