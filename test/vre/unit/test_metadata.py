...
def test_RepresentationMetadata_ctor(tmp_path: Path):
    metadata = RepresentationMetadata("repr_metadata", "run_id", tmp_path/"metadata.json", [0, 1, 2, 3, 4, 5])
...
def test_RepresentationMetadata_add_time_1(tmp_path: Path):
    ...
    loaded_json = json.load(open(metadata.disk_location, "r"))
    assert all(loaded_json["run_stats"][i]["duration"] == 0.03333333333333333 for i in ["0", "1", "2"])
    ...
    loaded_json = json.load(open(metadata.disk_location, "r"))
    assert all(loaded_json["run_stats"][i]["duration"] == 0.03333333333333333 for i in ["0", "1", "2"])
    assert all(loaded_json["run_stats"][i]["duration"] == 0.5 for i in ["3", "4"])
...
def test_RepresentationMetadata_add_time_in_threads(tmp_path: Path):
    def worker_fn(thread_ix: int, n_threads: int, tmp_path: Path):
        metadata = RepresentationMetadata("repr_metadata", f"run_id_{thread_ix}",
                                          tmp_path / "metadata.json", N := range(100))
        for i in N:
    ...
    for i in range(100):
        assert loaded_json["run_stats"][str(i)]["duration"] == i % n_threads
        assert loaded_json["run_stats"][str(i)]["run_id"] == f"run_id_{i % n_threads}"
...
def test_RunMetadata_two_representations(tmp_path: Path):
    ...
    metadata_r1 = RepresentationMetadata("r1", "run_id", tmp_path/"r2_metadata.json", [0, 1, 2, 3, 4, 5])
    metadata_r2 = RepresentationMetadata("r2", "run_id", tmp_path/"r1_metadata.json", [0, 1, 2, 3, 4, 5])
