...
def get_pca_data(img_hsv, dim_h=15, dim_s=11, dim_v=7, window=16, sample_step=30):
    ...
    xs, ys = torch.meshgrid(torch.arange(dw, nx - dw, dw), torch.arange(dw, ny - dw, dw))
    ...
    xs, ys = torch.meshgrid(torch.arange(dw, nx - dw, sample_step), torch.arange(dw, ny - dw, sample_step))
    ys, xs = ys.flatten(), xs.flatten()
