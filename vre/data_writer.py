...
class DataWriter:
    ...
    - export_npz Whether to export the npz files
    ...
    def __init__(self, output_dir: Path, representations: list[str], output_dir_exists_mode: str,
                 export_npz: bool, export_png: bool, binary_format: str = "npz", image_format: str = "png"):
        assert export_npz + export_png > 0, "At least one of export modes must be True"
        ...
        assert binary_format in ("npz", "npy", "npz_compressed"), binary_format
        assert image_format in ("png", ), image_format
        self.representations = representations
        self.output_dir = output_dir
        self.output_dir_exists_mode = output_dir_exists_mode
        self.export_npz = export_npz
        self.export_png = export_png
        self.export_binary = self.export_npz # TODO
        self.export_image = self.export_png # TODO
        self.binary_format = binary_format # TODO
        self.image_format = image_format # TODO
    ...
    def write(self, name: str, y_repr: ReprOut, imgs: np.ndarray | None, l: int, r: int):
        ...
        for i, t in enumerate(range(l, r)):
            if self.export_npz:
                if not (npz_path := self._path(name, t, "npz")).exists():
                    np.savez(npz_path, y_repr.output[i])
                    if (extra := y_repr.extra) is not None and len(y_repr.extra) > 0:
                        ...
                        np.savez(npz_path.parent / f"{t}_extra.npz", extra[i])
            if self.export_png:
                if not (png_path := self._path(name, t, "png")).exists():
                    image_write(imgs[i], png_path)

    def all_batch_exists(self, representation: str, l: int, r: int) -> bool:
        """true if all batch [l:r] exists on the disk"""
        for ix in range(l, r):
            if self.export_npz and not self._path(representation, ix, "npz").exists():
                return False
            if self.export_png and not self._path(representation, ix, "png").exists():
                return False
    ...
    def _make_dirs_one_reprsentation(self, representation: str):
        if self.export_npz:
            npz_dir = self.output_dir / representation / "npz"
            if npz_dir.exists() and not is_dir_empty(npz_dir, "*.npz"):
                if self.output_dir_exists_mode == "overwrite":
                    logger.debug(f"Output dir '{npz_dir}' already exists, will overwrite it")
                    shutil.rmtree(npz_dir)
                else:
                    assert self.output_dir_exists_mode != "raise", \
                        f"'{npz_dir}' exists. Set --output_dir_exists_mode to 'overwrite' or 'skip_computed'"
            npz_dir.mkdir(exist_ok=True, parents=True)

        if self.export_png:
            png_dir = self.output_dir / representation / "png"
            if png_dir.exists() and not is_dir_empty(png_dir, "*.png"):
                if self.output_dir_exists_mode == "overwrite":
