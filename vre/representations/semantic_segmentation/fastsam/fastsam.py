...
def postprocess(preds: tr.Tensor, inference_height: int, inference_width: int,
...
def postprocess2(proto: tr.Tensor, scaled_boxes: list, original_height: int,
    ...
    for i, scaled_box in enumerate(scaled_boxes):
        if len(scaled_box) == 0:  # save empty boxes
            ...
            masks = ops.process_mask_native(proto[i], scaled_box[:, 6:], scaled_box[:, :4],
                                            (original_height, original_width))
        results.append(Results(orig_img=orig_imgs[i], path=None, names=names, boxes=scaled_box[:, 0:6], masks=masks))
    return results
...
class FastSam(Representation):
    def __init__(self, variant: str, iou: float, conf: float, **kwargs):
        ...
        overrides = {"task": "segment", "imgsz": imgsz, "single_cls": False,
    ...
    def vre_setup(self, device: str, **kwargs):
    ...
    def make(self, t: slice) -> RepresentationOutput:
        ...
        scaled_boxes = postprocess(preds=tr_y, inference_height=tr_x.shape[2], inference_width=tr_x.shape[3],
                                   conf=self.conf, iou=self.iou, original_height=frames.shape[1],
                                   original_width=frames.shape[2])
        extra = [{"scaled_boxes": scaled_box.to("cpu").numpy()}
                 for scaled_box, _t in zip(scaled_boxes, range(t.start, t.stop))]
        assert len(tr_y[1]) == 3
    ...
    @overrides(check_signature=False)
    def make_images(self, t: slice, x: np.ndarray, extra: list[dict] | None) -> np.ndarray:
        assert extra is not None
        res: list[np.ndarray] = []
        tr_y = tr.from_numpy(x).to(self.device)
        orig_imgs = np.array(self.video[t])
        scaled_boxes = [tr.from_numpy(e["scaled_boxes"]).to(self.device) for e in extra]
        y_predictor = postprocess2(tr_y, scaled_boxes, original_height=orig_imgs.shape[1],
                                   original_width=orig_imgs.shape[2], orig_imgs=orig_imgs)
