...
def makeOutputDirs(args):
    ...
    for name, values in args.cfg["representations"].items():
        ...
        if Dir.exists():
            ...
            assert N == args.N, "Not all npz files were computed. Delete %s or set --N accordingly" % Dir
            skipRepresentations.append(name)
        else:
            Dir.mkdir(exist_ok=True)
            yaml.safe_dump(values, open(thisCfgFile, "w"))
    return skipRepresentations
...
def main():
    args = getArgs()
    skipRepresentations = makeOutputDirs(args)
    ...
    for t in trange(args.skip, args.skip + args.N):
        ...
        outPaths = {name : "%s/%s/%d.npz" % (args.outputDir, name, t-args.skip) for name in representations.keys()}
        outImagePath = args.outputDir / "collage" / ("%d.png" % (t-args.skip))

        # Topo sorted
        for name, representation in representations.items():
            # TODO: Load if in skipRepresentations
            depInputs = {k : finalOutputs[k] for k in args.cfg["representations"][name]["dependencies"]}
            # The current representation receives the current frame video[t] as well as the outputs of all dependencies
            ...
            np.savez_compressed(outPaths[name], resizedOutput)
